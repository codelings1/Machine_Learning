...
def cosine_similarity(u, v):
    ...
    norm_u = np.sqrt(sum([i*i for i in u]))
    
    # Compute the L2 norm of v (≈1 line)
    norm_v = np.sqrt(sum([i*i for i in v]))
...
paris = word_to_vec_map["boy"]
rome = word_to_vec_map["girl"]

print("cosine_similarity(father, mother) = ", cosine_similarity(paris, rome))
...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = (np.dot(e,g)/sum([i*i for i in g]))*g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = (np.dot(mu, bias_axis) / sum([i*i for i in g]))*bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (np.dot(e_w1,bias_axis)/sum([i*i for i in bias_axis]))*bias_axis
    e_w2B = (np.dot(e_w2,bias_axis)/sum([i*i for i in bias_axis]))*bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(np.abs(1 - np.square(np.linalg.norm(mu_orth, ord=2)))) * ((e_w1B - mu_B) / np.linalg.norm(e_w1 - mu_orth - mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1 - np.square(np.linalg.norm(mu_orth, ord=2)))) * ((e_w2B - mu_B) / np.linalg.norm(e_w2 - mu_orth - mu_B))
